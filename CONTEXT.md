...
- **Binding** — a way clients reach PgPaw's read/live capabilities. Optional, zero or more. Current bindings: HTTP (`HttpConfig`) and az-wire (`AzWireConfig`). Any binding works with any source.
...
- **PgPawBuilder** — the only composition entry point: `PgPaw::builder()`. Collects source, cache, auth, optional HTTP, zero-or-more az-wire configs, then `open()`.
- **AzWireConfig** — pair of `az_wire::NodeBuilder` + `az_wire::TopologyConfig` passed through verbatim. PgPaw never invents az-wire binding options.
...
- `binding/` — exposes capabilities over HTTP (`binding/http/`) or az-wire (`binding/az_wire.rs`).
...
- Lib owns PgPaw composition. az-wire crate owns az-wire topology. CLI only maps flags to the lib builder.
