...
`pgpaw serve` (and the default no-subcommand invocation) SHALL map to `PgPaw::builder().source(Source::replica(ReplicaSource{..})).cache(..).auth(..).http(HttpConfig{..})`, adding `.az_wire(NodeBuilder::new(node).insecure_accept_declared_peer_identities(), TopologyConfig::host(HostConfig::new(addr)))` only when `--az-wire-port` is set. Existing flag names, env vars, and defaults SHALL be preserved.
...
- THEN the builder opens with a replica source and an HTTP binding on `127.0.0.1:8080` and no az-wire binding

#### Scenario: Serve with az-wire host

- WHEN `pgpaw serve --az-wire-port 8788 --az-wire-host 127.0.0.1 --az-wire-node pgpaw ..` runs
- THEN the builder additionally starts one az-wire host topology on `127.0.0.1:8788` with node identity `pgpaw`

### Requirement: Primary Maps To Primary Builder

`pgpaw primary` SHALL map to `PgPaw::builder().source(Source::primary(PrimarySource{..}))`, adding `.az_wire(NodeBuilder::new(node).insecure_accept_declared_peer_identities(), TopologyConfig::parent(ParentLink::unix(parent_node, parent_unix)))` only when the az-wire parent flags are set. No HTTP flags are exposed for `primary` in this change.
...
#### Scenario: Primary as az-wire child

- WHEN `pgpaw primary .. --az-wire-node pgpaw --az-wire-parent-node worldant --az-wire-parent-unix /tmp/worldant.sock` runs
- THEN the builder additionally starts one az-wire parent-link topology connecting to `worldant` over the unix socket
