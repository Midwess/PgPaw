...
- [x] 1.2 Define `CacheConfig` (Default = 256 MiB), `HttpConfig` (server-gated, `SocketAddr` + `cors_origin`), `AzWireConfig` (az-wire-gated), `Source`/`ReplicaSource`/`PrimarySource` (+ `PrimarySource::embedded`, `Source::replica`/`primary`), relocated `UpstreamConfig` field-block (sslmode defaulted `"disable"`)
...
- [x] 2.1 Re-gate `CdcBridge::primary`/`publish` (src/cdc.rs) and `PrimaryObserver` (src/primary.rs) from `#[cfg(feature = "az-wire")]` to `#[cfg(feature = "read")]`
...
- [x] 5.1 `register_az_wire` → `pub(crate)`; drop `pub use` from `lib.rs`
- [x] 5.2 `open()` starts each `AzWireConfig` (`register_az_wire(node, read.clone()).build()?.start_topology(topology)`), errors mapped to `CacheError::lifecycle(Topology, _)`, rollback of started bindings + core on failure
- [x] 5.3 `wait()` selects biased over HTTP future + each topology `.wait()`; `shutdown()` stops topologies after HTTP, before source teardown; replica+az-wire and primary+az-wire builder tests

## 6. CLI Remap

- [x] 6.1 Move `shutdown_signal` from `lib.rs` to `main.rs` (move the `sigterm_completes_the_production_signal_wait` test from `az_wire.rs` along with it)
- [x] 6.2 `ServeOptions` maps to `Source::replica(ReplicaSource)` + `CacheConfig` + `AuthConfig` + `HttpConfig` + optional `.az_wire(NodeBuilder::new(node).insecure_accept_declared_peer_identities(), TopologyConfig::host(HostConfig::new(addr)))` when `--az-wire-port` set
...
- [x] 7.4 Remove orphaned imports; verify feature matrix builds: `--no-default-features --features read`, `--features server`, `--features az-wire`, `--all-features`
...
- [x] 8.4 Rewrite `src/az_wire.rs` inline tests that used old primary API
- [x] 8.5 Add `wait()`/`shutdown()` ordering tests across combos: replica+http, replica+az-wire, primary+http, primary+az-wire, primary-only

## 9. Documentation

- [x] 9.1 Update README examples to builder API (serve/primary/az-wire snippets)
...
- `tests/topology_benchmark.rs` uses raw `az_wire::Node` — confirm unaffected, no rewrite.
...
- `wait()` takes `&mut self` (actix Server task + `AzWireTopology::wait` poll by `&mut`); callers bind `let mut pgpaw`.
...
- CLI `primary` gains `--database` and `--az-wire-node`/`--az-wire-parent-node`/`--az-wire-parent-unix` flags (dream CLI shape); old flags unchanged.
...
- Verified green: 48 lib tests, 9+1 bin tests, 9 `tests/primary.rs` tests, feature matrix (`none`/`read`/`server`/`az-wire`/`--all-features --all-targets --workspace`) zero warnings.

### Code review (2026-07-16, 4 core agents, threshold 80) — 3 findings, all fixed

1. **[85] `PgPaw::wait()` cancellation-unsafety** (history-analyzer, vs commit 3e212b4): polling `topology.wait()` futures take()s az-wire's parent-link `JoinHandle` at first poll; dropping `wait()` (signal winning `run_pgpaw`'s select) made a later `shutdown()` silently skip awaiting the child task. Fixed by mirroring 3e212b4: `wait()` polls sync `is_finished()` (10ms sleep loop, non-consuming) and calls `topology.wait()` only after completion, so errors still surface through `wait()` and `shutdown()` always finds an intact task. Regression test: `interrupted_wait_still_shuts_down_a_parent_linked_child`.
