...
Scope: Replace PgPaw's three fragmented entrypoints (`run`/`run_until`, `open_primary`/`run_primary`, `register_unb`) with one `PgPaw::builder()` composition API — single `Source` (replica|primary) + optional bindings (HTTP, unb) + instance-based runtime, no global singleton, hard-cut of old API, CLI reduced to flag→builder mapping.

## Project Context
- **Tech Stack**: Rust 1.85, actix-web 4, tokio, `pglite-rs` (path dep on `../pglite-rs/crates/pglite`, features `replica` + `multiple-process`), unb (path dep, workspace member excluded from `[workspace]`), sqlparser 0.52, moka 0.12, serde/serde_json, thiserror 2, jsonwebtoken 9.
- **Architecture Style**: Single-crate layered lib + thin CLI. Feature-gated modules (`read`, `server`, `unb`) compose a "read core" that is fed to zero-or-more "bindings" (HTTP, unb). Today the read core is built by two separate, divergent code paths (`Di::init` for replica, `PrimaryHandle::attach_child` for primary) rather than one shared constructor.
- **Key Directories**: `src/lib.rs` (entrypoints), `src/di.rs` (singleton + replica read-core construction), `src/primary.rs` (primary lifecycle + primary read-core construction + `PrimaryObserver`), `src/unb.rs` (unb service registration), `src/http/*` (actix handlers), `src/operations.rs` (`ReadOperations`, the read core's façade), `src/main.rs` (clap CLI).

## Similar Features Found

### 1. Primary's `attach_child` (src/primary.rs:86-144)
- **Pattern**: Builds a second, parallel "read core" (schema scan → `VersionIndex` → `CdcBridge::primary` → `LiveHub::start` → `Verifier::build` → `ReadOperations::primary`) then wires it directly into unb via `crate::register_unb(...).build()` and `start_topology`. Closest existing precedent for "build read core once, hand to a binding" — but primary-only and unb-only; no HTTP binding over a primary today.
...
- **Pattern**: Full server lifecycle: `Di::init` → bind HTTP → optionally build+start unb topology → `tokio::select!` race between shutdown signal, HTTP future, and topology `.wait()` → ordered shutdown (HTTP `handle.stop(true)` → await HTTP future if not finished → topology `.shutdown()` → `Di::instance().shutdown()`).
- **Relevance**: Primary template for the new `PgPaw` runtime's `wait()`/`shutdown()` behavior — the select/shutdown-ordering logic moves onto the `PgPaw` struct, generalized to `Vec<UnbConfig>` (repeatable) and optional HTTP untied from `Di`.
...
| unb binding | `src/unb.rs` | `register_unb(builder, operations) -> NodeBuilder`, handler fns via `#[handler]` macro reading `State<ReadOperations>` | `register_unb`, `read`/`cursor`/`live` handlers |
...
- `unb::NodeBuilder`/`TopologyConfig`/`HostConfig`: `UnbConfig` wraps verbatim. Topology start (`build()` + `start_topology`) duplicated between `lib.rs::run_until` and `primary.rs::attach_child` with different error mapping (`CacheError::Config` vs `CacheError::lifecycle(LifecycleErrorKind::Topology, ...)`) — unified runtime should standardize on the lifecycle mapping.
...
- `ServerConfig`/`UpstreamConfig` (src/di.rs:18-44) — deleted; fields redistributed across `Source::replica(ReplicaSource)`, `HttpConfig`, `CacheConfig`, `AuthConfig`, `UnbConfig`.
...
4. Optional unb: `register_unb(NodeBuilder, Di::instance().operations().clone())` → `.build()` → `.start_topology(...)`.
...
**Primary + unb (current `attach_child`)**
...
3. `register_unb(...).build()` → `start_topology(topology)`.
4. Shutdown: topology → observer (cdc stop, unlisten) → `db.close()`.

**Divergence the unified read core must reconcile**: replica builds the read core eagerly inside `Di::init` (config → core, one shot); primary defers it to `attach_child`, only when unb is requested. The unified builder needs read-core construction as a single step, decoupled from any binding, running during `.open()` regardless of bindings.
...
| Feature gating | `server` for actix code, `unb` for unb code, `read` for read-core tree; layering `unb ⊃ read`, `server ⊃ read` |
...
- **Affected domains**: architectural/composition-layer change cutting across lifecycle/composition (`lib.rs`, `di.rs`, `primary.rs`, `unb.rs`), HTTP binding (`http/*`), auth (`AuthOutcome` `FromRequest`), CLI (`main.rs`). Does NOT touch `classify.rs`, `version.rs`, `cache.rs`, `live.rs`, `diff.rs`, `rows.rs`, `wire.rs`, `shadow.rs` internals.
...
| `register_unb` goes private but its registration logic is still needed | Must not break unb binding | Keep function body, drop `pub` + `pub use` re-export |
| Shutdown ordering differs between paths (server: HTTP→topology→replica/cdc/db; primary: topology→observer/cdc→db.close) | Unified `shutdown()`/`wait()` needs one canonical ordering correct for all combos, incl. HTTP-on-primary (no precedent) | Generalize: stop all bindings first (HTTP then unb topologies), then read-core internals (cdc/observer stop, replica.stop if replica, then db close) |
| `tests/primary.rs` (390 lines) + `src/unb.rs` test module use `open_primary`/`attach_child`/`EmbeddedVerifierConfig` directly | Near-total rewrite of those tests | Rewrite against `PgPawBuilder`/`PgPaw`; `recover_primary` tests stay (API kept); `tests/topology_benchmark.rs` uses raw `unb::Node`, unaffected |
